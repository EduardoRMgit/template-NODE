...
const [editedApellidos, setEditedApellidos] = useState('');
  ...
  useEffect(() => {
    ...
    const handleModify = (mesero) => {
      setEditingMesero(mesero);
      setEditedNombre(mesero.nombre);
      setEditedApellidos(mesero.apellidos);
      setShowModal(true);
    };
  
    const handleSaveEdit = async () => {
      try {
        const response = await axios.put(`http://localhost:4000/api/v1/meseros/${editingMesero.idMesero}`, {
          nombre: editedNombre,
        ...
        <Form>
          <Form.Group controlId="nombre">
            ...
            <Form.Control
              ...
              onChange={(e) => (editingMesero ? setEditedNombre(e.target.value) : setNombre(e.target.value))}
            />
          </Form.Group>
          <Form.Group controlId="apellidos">
            ...
            <Form.Control
              ...
              onChange={(e) => (editingMesero ? setEditedApellidos(e.target.value) : setApellidos(e.target.value))}
            />
          </Form.Group>
        </Form>
      </Modal.Body>
        ...
        <tbody>
          {meseros.map((mesero) => (
            ...
            <td style={{ textAlign: 'center' }}>
                ...
                <Button variant="success" style={{ marginRight: '10px' }} onClick={() => handleModify(mesero)}>
                Modificar
                </Button>
                <Button
                  variant="danger"
